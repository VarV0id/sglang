...
class ReqKvInfo:
    ...
    # Fork: when set, freezes the mamba ping-pong snapshot at the end-of-prefill
    # position so strip_thinking_cache can pair the prompt-prefix KV with a
    # consistent mamba state. Set in process_batch_result_prefill once prefill is
    # fully drained and the request will produce reasoning content
    # (require_reasoning + strip_thinking_cache). While set, decode-time mti
    # boundary updates are suppressed so the snapshot in
    # ping_pong[other(mamba_next_track_idx)] is preserved.
    mamba_prompt_anchor_seqlen: Optional[int] = None
...
class Req(ReqDllmMixin):
    ...
    def effective_kv_committed_len(self) -> int:
        ...
        # Fork: when extra_buffer mamba is enabled, the commit length must align
        # with the mamba snapshot position so cache_finished_req's invariant
        # `cache_len == page_aligned_len` holds. The snapshot is captured at
        # FLA-aligned positions, so return the anchor seqlen recorded at
        # end-of-prefill. Without anchoring, _cache_commit_len returns e.g. 1497
        # while mamba_last_track_seqlen advances to the next mti boundary (1536)
        ...
        if get_serving().strip_thinking_cache and self.reasoning_tokens > 0:
            if self.kv.mamba_prompt_anchor_seqlen is not None:
                return self.kv.mamba_prompt_anchor_seqlen
    ...
    def reset_for_retract(self):
        ...
        self.kv.mamba_prompt_anchor_seqlen = None
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def _mamba_radix_cache_v2_req_prepare_for_extend(
        ...
        if mask:
            ...
            req.kv.mamba_last_track_seqlen is None
            ...
            req.kv.mamba_last_track_seqlen = len(req.prefix_indices)
    ...
    def prepare_for_decode(self):
        ...
        if mamba_extra_buffer_enabled():
            ...
            # Fork: suppress kernel mti boundary writes for requests whose prompt
            # mamba anchor is pinned (strip_thinking_cache + reasoning). The anchor
            # is in ping_pong[other(mamba_next_track_idx)] and would be clobbered
            # at the next mti boundary if we left the mask True.
            anchored_mask_cpu = torch.tensor(
                [r.kv.mamba_prompt_anchor_seqlen is not None for r in self.reqs],
                dtype=torch.bool,
            )
            track_mask_cpu = track_mask_cpu & ~anchored_mask_cpu
