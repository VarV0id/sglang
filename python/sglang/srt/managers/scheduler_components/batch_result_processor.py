...
class SchedulerBatchResultProcessor:
            ...
            for i, (req, next_token_id) in enumerate(zip(batch.reqs, next_token_ids)):
                ...
                if req.inflight_middle_chunks <= 0:
                    ...
                    if req.beam_group is not None:
                        ...
                        # Strip-thinking-cache mamba anchor (fork): pin the
                        # end-of-prefill mamba snapshot so cache_finished_req can
                        # pair the prompt prefix KV with a consistent mamba state.
                        # Captured here because the prefill kernel just wrote the
                        # snapshot at mamba_last_track_seqlen into
                        # ping_pong[other(next_track_idx)]. Without this anchor the
                        # snapshot gets overwritten at the first mti boundary in
                        # decode. Gate on require_reasoning so non-reasoning
                        # requests retain full decode-time tracking.
                        ...
                        if (
                            ...
                            and req.kv.mamba_last_track_seqlen is not None
                            and req.kv.mamba_prompt_anchor_seqlen is None
                        ):
                            req.kv.mamba_prompt_anchor_seqlen = (
    ...
    def _mamba_prefix_cache_update(
        ...
        if req.kv.mamba_ping_pong_track_buffer is not None:
            ...
            # prompt anchor only when req.kv.mamba_last_track_seqlen is already
            ...
            # prefill-time site.
            if req.kv.mamba_prompt_anchor_seqlen is None and len(req.output_ids) <= 1:
                sa = get_global_server_args()
                if (
                    sa.strip_thinking_cache
                    and sa.enable_mamba_extra_buffer()
                    ...
                    req.kv.mamba_prompt_anchor_seqlen = req.kv.mamba_last_track_seqlen
            ...
            if req.kv.mamba_prompt_anchor_seqlen is not None:
                return
