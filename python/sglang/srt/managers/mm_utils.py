    ...
    **kwargs,
    ...
    if not hasattr(language_model, "pp_group") or language_model.pp_group.is_first_rank:
        if (
            ...
            # prealloc_deepstack (fork, upstream PR #16785): pre-allocated deepstack
            # embed buffer threaded in from PiecewiseCudaGraphRunner so
            # embed_mm_inputs can reuse it via .zero_() instead of torch.zeros() —
            # required to keep CUDA graph capture/replay address-stable (fixes
            # "PCG capture stream is not set" on Qwen3-VL-family models).
