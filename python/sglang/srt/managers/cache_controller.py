...
class HiCacheController:
    ...
    def attach_storage_backend(
        ...
        try:
            ...
            # Patch F (fork): explicit override wins; otherwise auto-resolve.
            # The ratio=1.0 fallback keeps L3 reads alive when host <= device
            # (upstream's 0.5*host formula still works there, but the override
            # lets operators raise the in-flight prefetch budget directly).
            if hicache_prefetch_capacity_tokens > 0:
                self.prefetch_capacity_limit = hicache_prefetch_capacity_tokens
            elif self.host_memory_mode == "buffer_only":
                ...
                # Budget speculative prefetch at half the host pool, leaving the
                # rest for the write-back staging path.
