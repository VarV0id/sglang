...
def build_hybrid_mamba_stack(
    *,
    ...
    # Alt B (fork): reserve overflow rows at the tail of the mamba host
    # pool's tensors so write_backup can fall through to a pre-allocated
    # ring when the LRU pool is full. Operator-tunable via
    # --mamba-overflow-size N on ServerArgs.
    if mtp_draft_device_pools:
        full_layer_mapping = _with_mtp_layer_mapping(
            full_layer_mapping,
            transfer_layer_start=transfer_layer_num,
    ...
    try:
        ...
        host_pool_group, cache_controller = build_anchor_sidecar_stack(
            ...
            hicache_prefetch_capacity_tokens=getattr(
                get_memory(), "hicache_prefetch_capacity_tokens", 0
            ),
        )
        radix_cache.full_kv_pool_host = host_pool_group.get_pool(PoolName.KV)
        radix_cache.token_to_kv_pool_host = host_pool_group
