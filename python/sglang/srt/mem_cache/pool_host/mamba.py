...
class MambaPoolHost(HostKVCache):
    def __init__(
        ...
        # so include them in the budget check (fork Alt B).
    ...
    def free(self, indices: torch.Tensor) -> int:
        ...
        # LRU pool. ``overflow_release`` is the correct release path for those.
        # In production we filter to avoid corrupting free_slots if a caller
        # misroutes a slot id.
