...
class ServerArgs:
    ...
    mamba_overflow_size: A[
        ...
        Arg(
            help=(
                "Fork Alt B: number of pinned overflow rows reserved at the tail of "
                "the mamba host pool's tensors. Used when the LRU mamba pool is full "
                "at write_backup time so the companion .mamba_*.bin file is still "
                "written and the KV write is not rolled back. Default 8 saturates "
                "~3 GB/s NVMe; raise to 16 if soak shows queueing. Set 0 to disable."
            ),
        ),
        NS("memory"),
    ] = 8
    hicache_prefetch_threshold: A[
        int,
        Arg(
            help=(
                "Minimum page-aligned new-input token count required for "
                "prefetch_from_storage to dispatch an L3 lookup. Lower-bounded "
                "internally to page_size. Lower (e.g. 16) lets smoke-test prompts "
                "exercise the L3 read path; raise to suppress lookup spam on "
                "trivially-short prompts. The storage-backend extra_config JSON key "
                "prefetch_threshold wins if both are set."
            ),
        ),
        NS("memory"),
    ] = 256
    hicache_prefetch_capacity_tokens: A[
        int,
        Arg(
            help=(
                "Override the L3 prefetch in-flight capacity (tokens). 0 = auto "
                "(upstream formula). When the auto formula clamps to 0 (e.g. "
                "hicache-ratio=1.0 where host == device), L3 reads are effectively "
                "disabled because prefetch_rate_limited() returns True immediately. "
                "Set to a positive integer (e.g. 65536) to unblock cross-session "
                "reuse on ratio=1.0."
            ),
        ),
        NS("memory"),
