...
class OpenAIServingChat(OpenAIServingBase):
    ...
    async def _generate_stream_content(
        ...
        # Fork: strip residual special-token *strings* (e.g. literal "<|im_start|>"
        # bytes hallucinated by the model) BEFORE the reasoning/tool parsers so
        # they see a clean stream. Streaming-safe — markers split across chunks
        # are buffered until completed. Per-index stripper state lives on the
        # serving instance (parallel-sampling choices are independent).
        if self._residual_marker_strings and getattr(request, "skip_special_tokens", True):
            stripper_dict = getattr(self, "_residual_stripper_dict", None)
            if stripper_dict is None:
                stripper_dict = self._residual_stripper_dict = {}
            stripper = stripper_dict.get(index)
            if stripper is None:
                stripper = stripper_dict[index] = StreamingResidualStringStripper(
                    self._residual_marker_strings
                )
            delta = stripper.feed(delta)
            if finish_reason_type is not None:
                delta += stripper.flush()
                stripper_dict.pop(index, None)
