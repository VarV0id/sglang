...
class TestQwen3CoderReasoningDetector(CustomTestCase):
    ...
    def test_streaming_no_stream_reasoning(self):
        ...
        # Mirrors Glm45Detector behavior (upstream base): the opening tag is
        # stripped from self._buffer in the non-streaming path, so the flush on
        # tool interruption matches detect_and_parse and does NOT carry the raw
        # <think> tag into reasoning_content.
        result = detector.parse_streaming_increment("<tool_call>do_thing")
        self.assertEqual(result.reasoning_text, "planning")
